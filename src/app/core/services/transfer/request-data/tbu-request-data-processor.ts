...
import {
  TBU_INCOMING_PURPOSE,
  TBU_OUTGOING_PURPOSE
...
export class TbuRequestDataProcessor implements RequestDataProcessor {
  ...
  public getCurrencyCode(): string {
    return this.commonRequestProcessor.getCurrencyCodeByPurpose(TBU_OUTGOING_PURPOSE);
  }

  public getRate(): CurrencyAmount {
    const currencyCode = this.commonRequestProcessor.getCurrencyCodeByPurpose(TBU_INCOMING_PURPOSE);
  ...
  private getAccountFromNumber(): string {
    return this.commonRequestProcessor.getAccountNumberByPurpose(TBU_OUTGOING_PURPOSE);
  }

  private getAccountToNumber(): string {
    return this.commonRequestProcessor.getAccountNumberByPurpose(TBU_INCOMING_PURPOSE);
  }
}
